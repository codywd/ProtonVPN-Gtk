...
class Handler:
    def __init__(self):
        ...
        with open("/home/" + currentUser + "/.pvpn-cli/pvpn-cli.cfg", "r") as f:
    ...
    def connectBtn(self, button):
        ...
        subprocess.Popen(
            [
                "protonvpn",
                "c",
                str(self.browseServer.get_active_id()),
                str(self.protocolSelection.get_active_id()),
            ]
        )
    ...
    def disconnectFunc(self):
        ...
        p = subprocess.Popen(["protonvpn", "d"], stdout=subprocess.PIPE, shell=False)
        time.sleep(5)
        p.kill()
        self.connectionProgress.stop()
    ...
    def updateBtn(self, button):
        subprocess.Popen(["protonvpn", "--update"])

    # Connect to fastest server
    def fastestServerBtn(self, button):
        self.reconnect()
        self.connectionProgress.start()
        subprocess.Popen(["protonvpn", "c", "-f"])

    # Connect to random server
    def randomServerBtn(self, button):
        self.reconnect()
        self.connectionProgress.start()
        subprocess.Popen(["protonvpn", "c", "-r"])
    ...
    def reconnect(self):
        if self.statusLabel.get_text() == "Connected":
            ...
            subprocess.check_call(["protonvpn", "d"])
